...
def init_kun(data, date):
    if date not in data:
        data[date] = {
            "1-smena": {
                "vaqt": "08:00 - 20:00",
                "yonildi": None, "ochildi": None, "ochish_sabab": None,
...
1. "type" quyidagilardan biri bo'lishi kerak:
   ...
   - "ochdi" — zavod to'xtadi, o'chdi, tugadi, yopildi, smena tugadi
...
4. "sabab" — to'xtalish sababi (faqat toxtalish_boshlandi uchun). FAQAT quyidagi kategoriyalardan birini ishlat:
   - "Elektr uzilishi" — svet o'chdi, elektr yo'q, quvvat yo'q
   - "Texnik nosozlik" — mashina buzildi, lenta uzildi, mexanizm ishlamayapti
   - "Texnika yetishmasligi" — lebxir yo'q, ekskavator yo'q, mashina kelmadi, texnika boshqa joyda
   - "Xom ashyo yo'qligi" — qum yo'q, material tugadi, xom ashyo kelmadi
   - "Ishchi sababi" — ishchi kelmadi, smena kelmadi, odam yo'q
   - "Ob-havo sharoiti" — yomg'ir, qor, bo'ron, ob-havo yomon
   - "Rejalashtirilgan to'xtatish" — texnik xizmat, remont, rejali to'xtatish
   - "Boshqa sabab" — boshqa barcha holatlar

5. "ochish_sabab" — zavod o'chganda (type=ochdi) sabab ko'rsatilgan bo'lsa qisqacha yoz, yo'q bo'lsa null
   Misol: "Smena tugadi", "Rejalashtirilgan to'xtatish", "Texnik sabab"

Faqat JSON qaytار, boshqa hech narsa yozma:
{"type": "...", "smena": "...", "vaqt": "...", "sabab": "...", "ochish_sabab": "..."}"""
...
def smena_blok(smena_nomi, smena):
    ...
    if smena.get("ochish_sabab"):
        lines.append(f"  📋 O'chish sababi: *{smena['ochish_sabab']}*")
    if jami_ish:
...
def sutka_hisobot_matn(data, hisobot_date):
    ...
    sof = round(jami_ish - jami_tox, 2)
    sof_foiz = round((sof / jami_ish) * 100, 1) if jami_ish > 0 else 0
    tox_foiz = round((jami_tox / jami_ish) * 100, 1) if jami_ish > 0 else 0

    lines.append(f"  ⏱ Ish vaqti: *{round(jami_ish,2)}h*")
    lines.append(f"  ✅ Sof ish vaqti: *{sof}h* ({sof_foiz}%)")

    if jami_tox_soni:
        lines.append(f"  ⚠️ To'xtalish: *{jami_tox_soni} ta* | *{round(jami_tox,2)}h* ({tox_foiz}%)")
        sabab_counter = defaultdict(lambda: {"soni": 0, "soat": 0.0})
        if kecha in data:
            for sn in ["1-smena", "2-smena"]:
                for tx in data[kecha].get(sn, {}).get("toxtalishlar", []):
                    if tx.get("boshlandi") and tx.get("tugadi"):
                        s = tx.get("sabab", "Boshqa sabab")
                        sabab_counter[s]["soni"] += 1
                        sabab_counter[s]["soat"] += time_diff(tx["boshlandi"], tx["tugadi"])
        if sabab_counter:
            lines.append("")
            lines.append("  📋 *Sabablari:*")
            for sabab, v in sorted(sabab_counter.items(), key=lambda x: -x[1]["soat"]):
                foiz = round((v["soat"] / jami_tox) * 100, 1) if jami_tox > 0 else 0
                lines.append(f"    • *{sabab}*: {v['soni']} ta | {round(v['soat'],2)}h | {foiz}%")
    else:
...
def oylik_hisobot_matn(data, month):
    ...
    lines = [
        ...
        f"✅ Sof ish vaqti: *{sof} soat* ({sof_foiz}%)",
        f"⚠️ To'xtalish vaqti: *{round(jami_tox,2)} soat* ({tox_foiz}%)",
        "━━━━━━━━━━━━━━━━━━",
        f"🔢 Jami to'xtalishlar: *{jami_tox_soni} ta*",
    ]

    if sabab_counter:
        lines.append("")
        lines.append("📋 *To'xtalish sabablari:*")
        for sabab, soat in sorted(sabab_counter.items(), key=lambda x: -x[1]):
            foiz = round((soat / jami_tox) * 100, 1) if jami_tox > 0 else 0
            soni = sum(
                1 for d in month_keys
                for sn in ["1-smena", "2-smena"]
                for tx in data[d].get(sn, {}).get("toxtalishlar", [])
                if tx.get("sabab") == sabab
            )
            lines.append(f"  • *{sabab}*: {soni} ta | {round(soat,2)}h | {foiz}%")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result["type"] == "yondi":
        ...
        ochish_sabab = result.get("ochish_sabab")
        if ochish_sabab:
            smena_data["ochish_sabab"] = ochish_sabab
        save_data(data)
        extra = ""
